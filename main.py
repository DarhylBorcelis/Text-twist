...
class Game_play:
    ...
    def Update_2(self, event):
        ...
        if event.type == pygame.MOUSEBUTTONDOWN:
            pos = event.pos
            for i, (img, rect, text_surface, text_rect) in enumerate(self.letters):
                if rect.collidepoint(pos):
                    char = self.choices[i]  # get the clicked letter
                    self.guest.append(char)  # add it to the guess

    def Draw(self, SCREEN):
        for img, rect, text_surface, text_rect in self.letters:
            SCREEN.blit(img, rect)
            SCREEN.blit(text_surface, text_rect)

        guess_text = "".join(self.guest)
        x_cord = (1200 - (len(self.choices) * 90)) // 2
        txt = FONT.render(guess_text, True, WHITE)
        txt = SCREEN.blit(txt, (x_cord, 413))
    ...
    def Update_ans(self):
        ...
        for i, char in enumerate(self.choices):
            ...
            self.letters.append((img, rect, text_surface, text_rect))
...
Level_back_btn = Button(690, -25, BUTTON_IMAGE, (100, 100),
                        "Back", key=pygame.K_ESCAPE)
    ...
    for event in pygame.event.get():
        ...
        if game_state == "Main_Menu":
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                ...
                if Level_animal_btn.event_handler() or Level_animal_btn.rect.collidepoint(pos):
                    game_category = "Animal"
                ...
                if Level_food_btn.event_handler() or Level_food_btn.rect.collidepoint(pos):
                    print("Food")
                    game_category = "Food"
                    CATEGORY = load_category(game_category)
                    game_level = 1
                ...
                if Level_place_btn.event_handler() or Level_place_btn.rect.collidepoint(pos):
                    game_category = "Place"
                    CATEGORY = load_category(game_category)
                    game_level = 1
